...
1. **BlockChain_Venture_Ca.sol** - Solidity smart contract (70 lines)
2. **BlockChain_Venture_Ca.abi.json** - Contract ABI (10 elements)
...
### MCP Server
5. **BlockChain_Venture_Ca_mcp_server.py** - Custom MCP server (152 lines)
...
- `calculateDividends()` - Read-only query
- `getContractDetails()` - Read-only query
- `invest()` - Payable transaction
- `qualifyIpo()` - State-changing transaction
- `redeemShares()` - State-changing transaction
- `withdrawDividends()` - State-changing transaction
